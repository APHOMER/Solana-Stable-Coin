use anchor_lang::prelude::*;

use state::*;
mod state;
use constants::*;
pub constants;
use instructions::*;
mod instructions;
use error::*;
mod error;


declare_id!("3jYnJ7ZFi9EXFh7fCEkj38H4mFZwgBrrZ2ejD7g9Ykxw");

#[program]
pub mod stable_coin {
    use super::*;

    pub fn initialize_config(ctx: Context<Initialize>) -> Result<()> {
        process_initialize_config(ctx)

        pub fn update_config(ctx: Context<updateConfig>, mint_health_factor: u64) -> Result<()> {
            process_update_config(ctx, mint_health_factor)
        }

        pub fn deposit_collateral_and_mint_tokens (
            ctx: Context<DepositCollateralAndMintTokens>,
            amount_collateral: u64,
            amount_to_mint: u64,
         ) -> Result<()> {
            process_deposit_collateral_and_mint_token(ctx: amount_collateral, amount_to_mint)
         }

         pub fn redeem_collateral_and_burn_tokens (
            ctx: CpiContext<RedeemCollateralAndBurnTokens>,
            amount_collateral: u64,
            amount_to_burn: u64,
         ) -> Result <()> {
            process_redeem_collateral_and_burn_tokens(ctx: amount_collateral, amount_to_burn)
         }
        
        pub fn liquidate(ctx: Context<Liquidate>, amount_to_burn: u64) -> Result<()> {
            process_liquidate(ctx, amount_to_burn)
        }
        
         Ok(())
    }
}



