...
describe("stable-coin", () => {
  ...
  const solanaPriceFeedAccount = pythSolanaReceiver.getPriceFeedAccountAddress(
    0,
    SOL_PRICE_FEED_ID
  );

  const [solanaPriceFeedAccount] = anchor.web3.PublicKey.findProgramAddressSync(
  ...
  it("Deposit Collateral and Mint USDC", async() => {
    ...
    const tx = await program.methods
      ...
      .accounts({ priceUpdate: solanaPriceFeedAccount })
      .rpc({ skipPreflight: true, commitment: "confirmed" });
      console.log("Your transaction signature", tx);
  });
  ...
  it("Redeem Collateral and Burn USDC", async () => {
    ...
    const tx = await program.methods
      ...
      .accounts({ priceUpdade: solanaPriceFeedAccount })
      .rpc({ skipPreflight: true, commitment: "confirmed" });
    console.log("your connection signature", tx);
  });

  it("Liquidate", async () => {
    const amountToBorn = 500_000_000;
    const tx = await program.methods
      .liquidate(new anchor.BN(amountToBorn))
      .accounts({ collateralAccount, priceUpdate: solanaPriceFeedAccount })
      .rpc({ skipPreflight: true, commitment: "confirmed" });
    console.log("Your transaction signature", tx);
  });
...
// function depositCollateralAndMintTokens(arg0: anchor.BN, arg1: anchor.BN) {
